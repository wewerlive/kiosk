...
export const ProjectItemBox = () => {
  ...
  return (
    <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg">
      <div className="grid grid-cols-4 gap-4">
        {siteSnap.designs?.map((design, index) => {
          return (
            <div
              key={index}
              className="flex flex-grow items-center justify-center h-44 rounded-lg bg-gradient-to-r from-gray-400 to-gray-500"
            >
              <button className="text-2xl text-gray-400 p-4 rounded-lg" onClick={(e)=>{
                e.preventDefault();
                siteState.selectedDesign = design;
                navigate(`/app/design/${design._id}`);
              }}>
